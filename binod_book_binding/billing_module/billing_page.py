...
# PROFESSIONAL POS BILLING UI – FULL WORKING VERSION (NO DOUBLE STOCK DEDUCTION)
...
from product_module.product_ops import (
    ...
    search_products,
    get_product_by_id
    # NOTE: update_product is NOT imported – stock is updated by create_bill()
)
...
class BillingPage(tk.Frame):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.t = T()
        super().__init__(master, bg=self.t["bg"], **kwargs)
    ...
    def _build(self):
        ...
        tk.Label(header, text="🧾 Billing / Point Of Sale",
                 font=("Segoe UI", 26, "bold"), bg=t["bg"], fg=t["text"]).pack(anchor="w")
        tk.Label(header, text="Professional Billing Management System",
                 font=("Segoe UI", 11), bg=t["bg"], fg=t["text2"]).pack(anchor="w")
        ...
        # LEFT PANEL
        left = tk.Frame(body, bg=t["card"], bd=0,
                        highlightthickness=1, highlightbackground=t["border"])
        left.grid(row=0, column=0, sticky="nsew", padx=(0, 12))
        # RIGHT PANEL
        right = tk.Frame(body, bg=t["card"], bd=0,
                         highlightthickness=1, highlightbackground=t["border"])
    ...
    def _build_left(self, panel):
        ...
        # CUSTOMER SECTION
        tk.Label(inner, text="👤 Customer Information", font=("Segoe UI", 18, "bold"),
                 bg=t["card"], fg=t["accent"]).pack(anchor="w", pady=(0, 14))
        ...
        tk.Label(customer_grid, text="Customer Name *", bg=t["card"],
                 fg=t["text2"], font=("Segoe UI", 10, "bold")).grid(row=0, column=0, sticky="w", pady=(0, 4))
        self.f_cust = tk.Entry(customer_grid, font=("Segoe UI", 12),
                               bg=t["entry_bg"], fg=t["entry_fg"], relief="flat",
                               insertbackground=t["text"])
        self.f_cust.grid(row=1, column=0, sticky="ew", padx=(0, 8), ipady=8)

        tk.Label(customer_grid, text="Phone Number", bg=t["card"],
                 fg=t["text2"], font=("Segoe UI", 10, "bold")).grid(row=0, column=1, sticky="w", pady=(0, 4))
        self.f_phone = tk.Entry(customer_grid, font=("Segoe UI", 12),
                                bg=t["entry_bg"], fg=t["entry_fg"], relief="flat",
                                insertbackground=t["text"])
        self.f_phone.grid(row=1, column=1, sticky="ew", ipady=8)

        tk.Label(customer_grid, text="Address", bg=t["card"],
                 fg=t["text2"], font=("Segoe UI", 10, "bold")).grid(row=2, column=0, sticky="w", pady=(12, 4))
        self.f_addr = tk.Entry(customer_grid, font=("Segoe UI", 12),
                               bg=t["entry_bg"], fg=t["entry_fg"], relief="flat",
                               insertbackground=t["text"])
        ...
        # PRODUCT SECTION
        tk.Label(inner, text="📦 Add Product", font=("Segoe UI", 18, "bold"),
                 bg=t["card"], fg=t["accent"]).pack(anchor="w", pady=(0, 12))
        ...
        tk.Label(search_frame, text="Search Product", bg=t["card"],
                 fg=t["text2"], font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        self.prod_search = tk.Entry(search_frame, font=("Segoe UI", 12),
                                    bg=t["entry_bg"], fg=t["entry_fg"], relief="flat",
                                    insertbackground=t["text"])
        ...
        tk.Label(qty_frame, text="Qty", bg=t["card"],
                 fg=t["text2"], font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(0, 4))
        self.qty_var = tk.IntVar(value=1)
        self.qty_spinbox = tk.Spinbox(qty_frame, from_=1, to=999, width=6,
                                      textvariable=self.qty_var, font=("Segoe UI", 12),
                                      bg=t["entry_bg"], fg=t["entry_fg"], relief="flat",
                                      justify="center")
        ...
        self.add_btn = tk.Button(add_btn_frame, text="➕ Add", bg="#16a34a",
                                 fg="white", activebackground="#15803d",
                                 activeforeground="white", relief="flat",
                                 cursor="hand2", font=("Segoe UI", 11, "bold"),
                                 padx=18, pady=9, command=self._add_to_cart)
        self.add_btn.pack()

        self.sel_info = tk.Label(inner, text="Double-click product to select",
                                 bg=t["card"], fg=t["text2"], font=("Segoe UI", 10))
        ...
        style.configure("Custom.Treeview", background="#ffffff",
                        foreground="#111827", rowheight=32,
                        fieldbackground="#ffffff", borderwidth=0,
                        font=("Segoe UI", 10))
        style.configure("Custom.Treeview.Heading", background="#374151",
                        foreground="white", relief="flat",
                        font=("Segoe UI", 10, "bold"))
        style.map("Custom.Treeview", background=[("selected", "#2563eb")],
                  foreground=[("selected", "white")])

        self.prod_tree = ttk.Treeview(table_frame, columns=columns,
    ...
    def _build_right(self, panel):
        ...
        tk.Label(top, text="🛒 Shopping Cart", font=("Segoe UI", 18, "bold"),
                 bg=t["card"], fg=t["accent"]).pack(side="left")
        tk.Button(top, text="🗑 Clear Cart", bg="#dc2626", fg="white",
                  relief="flat", cursor="hand2", font=("Segoe UI", 10, "bold"),
                  padx=14, pady=6, command=self._clear_cart).pack(side="right")
        ...
        self.item_count_label = tk.Label(total_inner, text="Items: 0",
                                         font=("Segoe UI", 11),
                                         bg="#f3f4f6", fg="#6b7280")
        self.item_count_label.pack(side="left")
        tk.Label(total_inner, text="GRAND TOTAL", font=("Segoe UI", 15, "bold"),
                 bg="#f3f4f6", fg="#111827").pack(side="left", padx=(30, 10))
        ...
        self.generate_btn = tk.Button(btn_frame, text="🖨 Generate Bill",
                                      bg="#2563eb", fg="white",
        ...
        self.new_btn = tk.Button(btn_frame, text="📝 New Bill",
                                 bg="#4b5563", fg="white",
                                 activebackground="#374151",
                                 activeforeground="white", relief="flat",
                                 cursor="hand2", font=("Segoe UI", 12, "bold"),
                                 padx=25, pady=12, command=self._new_bill)
    ...
    def _load_prod_tree(self, rows):
        ...
        for r in rows:
            ...
            self.prod_tree.insert("", "end",
                                  values=(r["product_id"], r["product_name"],
    ...
    def _select_product(self, event=None):
        ...
        self.sel_info.config(
            text=f"✅ Selected: {prod['product_name']} | Stock: {prod['quantity']}"
                 f"{stock_status} | Price: ₹ {prod['price']:.2f}",
            fg="#16a34a"
        )
    ...
    def _add_to_cart(self):
        ...
        if self._selected_prod["quantity"] == 0:
            ...
            return
        already_added = sum(item["quantity"] for item in self._cart
        ...
        if qty > remaining_stock:
            messagebox.showwarning(
                ...
                f"{self._selected_prod['product_name']}\n"
                f"Current stock: {self._selected_prod['quantity']}\n"
                f"Already in cart: {already_added}"
            )
            return
    ...
    def _edit_cart_quantity(self):
        ...
        tk.Label(dialog, text=f"Product: {item['product_name']}",
                 font=("Segoe UI", 11, "bold"), bg=self.t["bg"],
                 fg=self.t["text"]).pack(pady=10)
        tk.Label(dialog, text="New Quantity:", bg=self.t["bg"],
                 fg=self.t["text"]).pack()
        qty_var = tk.IntVar(value=item['quantity'])
        tk.Spinbox(dialog, from_=1, to=999, textvariable=qty_var,
                   font=("Segoe UI", 11), width=10).pack(pady=5)
        def save():
            new_qty = qty_var.get()
            ...
            self._refresh_cart()
            dialog.destroy()
        tk.Button(dialog, text="Save", bg="#2563eb", fg="white",
                  command=save, font=("Segoe UI", 10, "bold"),
    ...
    # =========================================================
    # GENERATE BILL – FIXED (only one stock deduction by create_bill)
    # =========================================================
    def _generate_bill(self):
        ...
        try:
            ...
            # create_bill() already deducts stock in the database
            ...
            # ✅ NO manual stock update – stock is already correct
            ...
            self._bill_counter += 1
            self._new_bill()
            self._refresh_prod_list()   # refresh billing page product list

            # Notify the main app (use this to refresh the Products page)
    ...
    def _show_bill_popup(self, bill_data):
        ...
        tk.Label(popup, text="📚 BINOD BOOK BINDING", font=("Segoe UI", 22, "bold"),
                 bg=self.t["bg"], fg=self.t["text"]).pack(pady=12)
        tk.Label(popup, text="Professional Book Binding Services",
                 font=("Segoe UI", 10), bg=self.t["bg"],
                 fg=self.t["text2"]).pack()
        ...
        tk.Label(details_frame, text=f"Bill No: {bill_data.get('bill_number', 'N/A')}",
                 bg=self.t["bg"], fg=self.t["text"],
                 font=("Segoe UI", 11, "bold")).pack(anchor="w")
        tk.Label(details_frame, text=f"Date: {bill_data.get('date', datetime.now().strftime('%Y-%m-%d %H:%M:%S'))}",
                 bg=self.t["bg"], fg=self.t["text2"],
                 font=("Segoe UI", 10)).pack(anchor="w")
        tk.Label(details_frame, text=f"Customer: {bill_data.get('customer_name', 'N/A')}",
                 bg=self.t["bg"], fg=self.t["text"],
                 font=("Segoe UI", 11)).pack(anchor="w")
        if bill_data.get('phone'):
            tk.Label(details_frame, text=f"Phone: {bill_data.get('phone', '')}",
                     bg=self.t["bg"], fg=self.t["text2"],
                     font=("Segoe UI", 10)).pack(anchor="w")
        if bill_data.get('address'):
            tk.Label(details_frame, text=f"Address: {bill_data.get('address', '')}",
                     bg=self.t["bg"], fg=self.t["text2"],
                     font=("Segoe UI", 10)).pack(anchor="w")

        tk.Frame(popup, bg=self.t["border"], height=1).pack(fill="x", padx=20, pady=10)

        text = tk.Text(popup, font=("Consolas", 11), bg="white", fg="black",
                       wrap=tk.WORD)
        ...
        for i, item in enumerate(bill_data.get("items", []), 1):
            bill_text += (f"{i}. {item['product_name']}\n"
                          f"   {item['quantity']} x ₹ {item.get('price', 0):,.2f}"
                          f" = ₹ {item['subtotal']:,.2f}\n\n")
        bill_text += ("=" * 50 +
        ...
        tk.Button(button_frame, text="📄 Export PDF", bg="#2563eb", fg="white",
                  activebackground="#1d4ed8", font=("Segoe UI", 11, "bold"),
                  relief="flat", padx=20, pady=10, cursor="hand2",
                  command=export_pdf).pack(side="left", padx=5)
        tk.Button(button_frame, text="🖨 Print", bg="#059669", fg="white",
                  activebackground="#047857", font=("Segoe UI", 11, "bold"),
                  relief="flat", padx=20, pady=10, cursor="hand2",
                  command=lambda: messagebox.showinfo("Print",
                                                      "Print functionality coming soon!")
                  ).pack(side="left", padx=5)
        tk.Button(button_frame, text="✖ Close", bg="#6b7280", fg="white",
                  activebackground="#4b5563", font=("Segoe UI", 11, "bold"),
                  relief="flat", padx=20, pady=10, cursor="hand2",
    ...
    def _new_bill(self):
        ...
        self.prod_search.delete(0, tk.END)
        self.sel_info.config(text="Double-click product to select",
                             fg=self.t["text2"])
        self._selected_prod = None
        self.add_btn.config(bg="#16a34a")
        self._refresh_prod_list()
    ...
    # =========================================================
    # REFRESH (Called externally, e.g., from main app)
    # =========================================================
    def refresh(self):
        self._refresh_prod_list()
        if self._cart:
